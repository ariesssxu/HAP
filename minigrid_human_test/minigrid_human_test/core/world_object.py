...
from minigrid_human_test.core.constants import (
    COLOR_TO_IDX,
    COLORS,
    IDX_TO_COLOR,
    IDX_TO_OBJECT,
...
from minigrid_human_test.utils.rendering import (
    fill_coords,
    point_in_circle,
    point_in_line,
    point_in_rect,
...
class Key(WorldObj):
    ...
    def render(self, img):
        ...
        # Define the points of a five-pointed star (pentagram)
        # Using relative coordinates (0-1) within the cell
        star_points = [
            (0.50, 0.15),  # Top point
            (0.61, 0.40),  # Right middle
            (0.88, 0.40),  # Right bottom
            (0.66, 0.60),  # Bottom right
            (0.75, 0.85),  # Bottom point
            (0.50, 0.70),  # Bottom left
            (0.25, 0.85),  # Left bottom
            (0.34, 0.60),  # Left middle
            (0.12, 0.40),  # Left top
            (0.39, 0.40)   # Right top
        ]
        
        # Draw the star by filling the polygon defined by the points
        fill_coords(img, point_in_polygon(star_points), c)
