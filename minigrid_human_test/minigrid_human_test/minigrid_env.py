...
from minigrid_human_test.core.actions import Actions
from minigrid_human_test.core.constants import COLOR_NAMES, DIR_TO_VEC, TILE_PIXELS
from minigrid_human_test.core.grid import Grid
from minigrid_human_test.core.mission import MissionSpace
from minigrid_human_test.core.world_object import Point, WorldObj
...
class MiniGridEnv(gym.Env):
    ...
    def step(
        ...
        # Handle movement actions (up is forward, down is backward, left, right)
        if action in [self.actions.backward, self.actions.forward, self.actions.left, self.actions.right]:
            # Calculate delta based on the action
            dx, dy = 0, 0
            if action == self.actions.forward:
                dx, dy = 0, -1
                self.agent_dir = 3  # Set direction to up
            elif action == self.actions.backward:
                dx, dy = 0, 1
                self.agent_dir = 1  # Set direction to down
            elif action == self.actions.left:
                dx, dy = -1, 0
                self.agent_dir = 2  # Set direction to left
            elif action == self.actions.right:
                dx, dy = 1, 0
                self.agent_dir = 0  # Set direction to right
            ...
            fwd_cell = self.grid.get(new_x, new_y)

            # Move the agent if the cell is passable
            if fwd_cell is None or fwd_cell.can_overlap():
                self.agent_pos = target_pos

            # Check for goal or lava
