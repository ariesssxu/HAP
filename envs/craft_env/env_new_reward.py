...
class CraftLab(object):
  ...
  def __init__(self,
    ...
    self._legal_tasks = legal_tasks
    if self._legal_tasks is not None:
        self._legal_tasks = [util.parse_fexp(t)[1] for t in self._legal_tasks]
  ...
  def _get_reward(self):
    ...
    # print(self.world.cookbook.index)
    # print(self._current_state.inventory)
    unlocked = {
        i for i, count in enumerate(self._current_state.inventory)
        if count > 0 and i not in self._unlocked and (self._legal_tasks is None or i in self._legal_tasks)}
    if unlocked:
