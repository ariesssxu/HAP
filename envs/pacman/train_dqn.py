...
def main() -> None:
    task = "envs/PacMan-v0"
    render_mode = "symbol"
    train_flag = "dqn-symbol-news"
    lr, epoch, batch_size = 1e-3, 100, 64
    train_num, test_num = 128, 128
    gamma, n_step, target_freq = 0.999, 3, 320
    buffer_size = 500000
    eps_train, eps_test = 0.25, 0.05
    step_per_epoch, step_per_collect = 100000, 128
    ...
    with open(f"{log_path}/hyperparameters.json", "w") as f:
        json.dump({
            "lr": lr,
            "epoch": epoch,
    ...
    collector.collect(n_episode=2000, render=1 / 35)
