...
class Env(BaseClass):
  ...
  def __init__(
      ...
      reward=True, length=10000, seed=None, tasks=None, invincible=False):
  ...
  def action_space(self):
    ...
    # for td3
    # return BoxSpace(0, 1, (len(constants.actions),), np.float32)
  ...
  def reset(self, task=None):
    ...
    self._update_time()
    self._player = objects.Player(self._world, center, invincible=self._invincible)
  ...
  def step(self, action):
    ...
    self._last_health = self._player.health
    unlocked = {
        name for i, (name, count) in enumerate(self._player.achievements.items())
        if count > 0 and name not in self._unlocked and (self._legal_tasks is None or i in self._legal_tasks)}
    if unlocked:
